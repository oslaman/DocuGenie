...
// export async function reassignChildren(db, nodeId) {
//     const ids = await db.query(`SELECT id FROM rules WHERE parent_id = $1`, [nodeId]);
//     console.table(ids.rows);
//     ids.rows.forEach(async row => {
//         await db.query(`UPDATE rules SET parent_id = $1 WHERE parent_id = $2`, [row.id, nodeId]);
//     });
// }

// export async function removeParent(db, nodeId) {
//     await db.query(`UPDATE rules SET parent_id = NULL WHERE id = $2`, [nodeId]);
// }